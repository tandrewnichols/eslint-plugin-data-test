  ...
  create: function(context) {
    ...
    return {
      JSXOpeningElement: (node) => {
        ...
        if (hasProp(attributes, 'onKeyUp') && !hasAnyProp(attributes, testAttribute)) {
          context.report({
            node,
            message: `Elements with an onKeyUp handler must have one of the following attributes: ${ testAttribute.join(', ') }.`
          });
        }
      }
    };
